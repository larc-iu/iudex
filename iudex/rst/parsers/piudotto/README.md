...
- **Split scoring.** With `decoder_layers = 0` (default), each candidate split
  ...
  self-attention over the prior decisions, cross-attention over the EDU reprs)
  and a pointer head scores the split from the history-conditioned query, the
  non-RNN analog of dmrst's GRU decoder. `decoder_order` threads that history in
  `dfs` (left-first preorder, matches dmrst) or `bfs` (level order,
  coarse-to-fine) visitation order.
...
- **Training objective.** Per-node teacher-forced cross-entropy on splits and
  ...
  decoder pass over the decision sequence; the loss decomposition is unchanged.
