...
class _DecoderOnlySRDataset(Dataset):
...
def _build_dataset(model: DecoderOnlySRParser, pairs: list[tuple[str, RstTree]]) -> tuple[_DecoderOnlySRDataset, int]:
    items: list[dict] = []
    dropped = 0
    for _, tree in pairs:
    ...
    return _DecoderOnlySRDataset(items), dropped


def _build_optimizer(model: DecoderOnlySRParser, cfg: DecoderOnlySRConfig):
...
def train(cfg: DecoderOnlySRConfig) -> None:
    ...
    model = DecoderOnlySRParser(cfg).to(device)
...
def main():
    parser = argparse.ArgumentParser(description="Train the decoder_only_sr parser")
    parser.add_argument("config", help="Path to a jsonnet config file")
    args = parser.parse_args()
    cfg = DecoderOnlySRConfig.from_dict(Params.from_file(args.config).as_dict(quiet=True))
    train(cfg)
