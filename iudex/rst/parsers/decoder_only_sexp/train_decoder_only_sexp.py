...
class _DecoderOnlySexpDataset(Dataset):
...
def _build_dataset(
    model: DecoderOnlySexpParser, pairs: list[tuple[str, RstTree]]
) -> tuple[_DecoderOnlySexpDataset, int]:
    items: list[dict] = []
    dropped = 0
    for _, tree in pairs:
    ...
    return _DecoderOnlySexpDataset(items), dropped


def _build_optimizer(model: DecoderOnlySexpParser, cfg: DecoderOnlySexpConfig):
...
def train(cfg: DecoderOnlySexpConfig) -> None:
    set_seeds(cfg.seed)
    ...
    model = DecoderOnlySexpParser(cfg).to(device)
    ...
    if dropped > 0:
        warn(
            f"Dropped {dropped}/{len(train_pairs)} training trees whose source or target overflowed "
            f"max_input_length={cfg.max_input_length} / max_output_length={cfg.max_output_length}."
        )
    ...
    # ceil, not floor: the loop fires a trailing optimizer step on the final
    # partial accumulation window (`(batch_idx + 1) == len(train_loader)`), so
    # a floor would undercount steps_per_epoch and shorten the LR schedule.
...
def main():
    parser = argparse.ArgumentParser(description="Train the decoder_only_sexp parser")
    parser.add_argument("config", help="Path to a jsonnet config file")
    args = parser.parse_args()
    cfg = DecoderOnlySexpConfig.from_dict(Params.from_file(args.config).as_dict(quiet=True))
    train(cfg)
