...
class DecoderOnlySexpConfig(FromParams):
    ...
    # Single-stream layout: source + tree + specials share one sequence. The
    # trainer enforces the combined length internally.
    max_input_length: int = 3072
    max_output_length: int = 5120
    gradient_checkpointing: bool = False

    # Parser-kind tag distinguishing this config from the seq2seq_sexp side.
    # Reading the value is a no-op.
    causal_mode: bool = True

    # LoRA. Null = full fine-tuning. When set, the base LM is frozen and only
    ...
    # S-expression knobs. "postorder" or "preorder".
    # `use_copy` is the registry's signature_field.
