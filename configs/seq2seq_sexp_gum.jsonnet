// seq2seq_sexp trained on GUM 12.1 (fine relations). Canonical config:
// every field on `Seq2SeqSexpConfig` is set explicitly, even if at the
// dataclass default, so the file is self-documenting.
{
    // Data
    train_dir: 'data/gum_12.1.0_notok/train',
    dev_dir: 'data/gum_12.1.0_notok/dev',
    test_dir: 'data/gum_12.1.0_notok/test',
    relation_types: null,   // inferred at train time from train_dir+dev_dir
    relation_map: null,     // GUM uses its native fine relation set

    // Model
    model_name: 'google/t5gemma-2-1b-1b',
    // Sexp serialization is ~2x as long as flat shift-reduce (each leaf
    // adds an open+close pair, each internal node likewise), so the
    // target headroom is bumped vs seq2seq_sr.
    max_input_length: 3072,
    max_output_length: 8192,
    gradient_checkpointing: false,

    peft: {
        r: 8,
        alpha: 16,
        dropout: 0.10,
        target_modules: 'all-linear',
        bias: 'none',
        dora: false,
        // No effect under the default use_copy=true (small action head, frozen
        // base embedding + new-rows Parameter). Honored only under use_copy=false.
        modules_to_save: ['embed_tokens'],
        train_only_new_embedding_rows: true,
    },

    // Training
    lr: 3e-4,
    weight_decay: 0.05,
    curriculum: { epochs: 200 },
    batch_size: 1,
    grad_accum: 8,
    optimizer: 'adafactor',
    num_warmup_steps: null,
    max_grad_norm: 1.0,
    amp: true,
    patience: 5,
    log_every: 5,
    checkpoint_dir: 'checkpoints',
    run_name: null,
    seed: 42,
    val_metric_name: 'e2e_full_f1',
    action_loss_weight: 1.0,
    // Applied uniformly at this value in both use_copy modes (no auto-scaling).
    // Under use_copy=false this is a known confound across the ~100-class head
    // and the full-vocab head.
    label_smoothing: 0.1,

    // Dev eval
    dev_max_docs: null,
    dev_batch_size: 1,

    // Decoding
    num_beams: 4,
    use_validity_constraints: true,
    eval_decode_greedy: true,
    min_edu_length: 1,

    // Sexp-specific
    traversal_order: 'postorder',
    use_copy: true,
    // Only meaningful when use_copy=false. true = hard-mask content
    // positions to source_ids[cursor] (COPY-via-constraint). false =
    // free content generation (Hu and Wan 2023's apparent setup).
    constrain_content: true,
}
